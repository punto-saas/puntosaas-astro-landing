...
const schema = z.object({
  ...
  (data) => {
    if (data.traction !== "Sin ventas aún") {
      return data.ticketAvg !== undefined && data.mrr !== undefined;
    }
    return true;
  },
  {
    message: "Completa ticket promedio y MRR",
    path: ["ticketAvg"],
  }
);
...
export default function PuntoSaasForm() {
  ...
  const [traction, setTraction] = useState(initialState.traction);
  const {
    register,
    handleSubmit,
  ...
  // Watch traction value for conditional rendering
  const tractionValue = watch("traction", traction);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 bg-white/90 border border-[var(--color-border)] rounded-2xl shadow-lg px-4 md:px-10 py-8 md:py-12 relative overflow-hidden" noValidate>
      ...
      {/* Sección 1: Datos de Tu Startup */}
      <div className="mb-10 p-6 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)]">
        <h3 className="text-xl font-bold mb-6 text-[var(--color-primary)]">Datos de Tu Startup</h3>
        ...
        <div className="flex flex-col gap-2 mb-4">
          ...
          <select
            ...
            onChange={e => {
              ...
              setValue("traction", e.target.value);
              if (e.target.value === "Sin ventas aún") {
                setValue("ticketAvg", undefined);
                setValue("mrr", undefined);
              }
            }}
          >
      ...
      {/* Sección 2: Información de tu ICP */}
      <div className="mb-10 p-6 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)]">
        <h3 className="text-xl font-bold mb-6 text-[var(--color-primary)]">Información de tu ICP</h3>
        <div className="flex flex-col gap-2 mb-4">
      ...
      {/* Sección 3: Contacto */}
      <div className="mb-4 p-6 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)]">
        <h3 className="text-xl font-bold mb-6 text-[var(--color-primary)]">Contacto</h3>
        <div className="flex flex-col gap-2 mb-4">
